...
def _kenburns(idx: int, dur: float, zoom_in: bool) -> str:
    """Bitta rasm uchun filtr zanjiri."""
    ...
    # kichik gorizontal siljish ham qo'shamiz
    drift = random.choice(["-", "+"])
    x = f"iw/2-(iw/zoom/2){drift}(on/{frames})*40"
    return (
        f"[{idx}:v]scale={config.W * 3 // 2}:{config.H * 3 // 2}:force_original_aspect_ratio=increase,"
        f"crop={config.W * 3 // 2}:{config.H * 3 // 2},"
